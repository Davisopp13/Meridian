...
const COLS = [
  { key: 'date', label: 'DATE', color: 'var(--text-pri)', bold: true },
  ...
  { key: 'total', label: 'TOTAL', color: 'var(--text-pri)', bold: true },
  { key: 'totalActivity', label: 'TOTAL ACTIVITY', color: 'var(--text-pri)', bold: true },
];
        ...
        return (
          <div key={row.date} style={rowStyle}>
            {COLS.map(col => {
              ...
              const isZero = val === 0;
              const cellColor = isZero && col.key !== 'date' ? 'var(--text-dim)' : col.color;
